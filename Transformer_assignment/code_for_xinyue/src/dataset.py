...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        inp = self.data[idx]
        trunc_len = np.random.randint(4, int(self.block_size * 7 / 8))
        inp_trunc = list(inp[:trunc_len])
        mask_len = np.random.binomial(n=trunc_len-3, p=(trunc_len-4)/(4*(trunc_len-3))) + 1  # binomial to determine mask length
        mask_start_loc = np.random.randint(1, trunc_len-mask_len)
        mask_end_loc = mask_start_loc+mask_len
        inp_x = inp_trunc[:mask_start_loc] + [self.MASK_CHAR] + inp_trunc[mask_end_loc:] \
                + [self.MASK_CHAR] + inp_trunc[mask_start_loc:mask_end_loc]
        x_seq = inp_x + [self.PAD_CHAR] * (self.block_size-len(inp_x)-1)
        y_seq = x_seq[1:] + [self.PAD_CHAR]
        x = torch.tensor([self.stoi[c] for c in x_seq], dtype=torch.long)
        y = torch.tensor([self.stoi[c] for c in y_seq], dtype=torch.long)
        return x, y # len(x)=len(y)=self.block_size-1
